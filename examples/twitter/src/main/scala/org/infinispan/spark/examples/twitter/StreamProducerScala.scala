...
object StreamProducerScala {
   def main(args: Array[String]) {
      if (args.length < 4) {
         println("Usage: StreamProducerScala <twitter4j.oauth.consumerKey> <twitter4j.oauth.consumerSecret> <twitter4j.oauth.accessToken> <twitter4j.oauth.accessTokenSecret> <infinispan-server>")
         System.exit(1)
      }
